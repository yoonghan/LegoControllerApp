...
class ListView extends React.PureComponent {
  ...
  _onCompletedChange = (dataItem, index) => {
    ...
    this._moveOrderItem(this, fromIndex, toIndex);
  }


  _moveOrderItem = (listView, fromIndex, toIndex) => {
    Utils.move(dataListOrder, parseInt(fromIndex), parseInt(toIndex));
    //if (listView.forceUpdate) listView.forceUpdate();
  }

  _renderRow = (dataItem, section, index) => (
    <ListViewItem data={dataItem} dataIndex={index} onCompletedChange={this._onCompletedChange}/>
  )
    ...
    if (this.state.dataList.length) {
      listView = (
        <SortableListView
          ...
          order={dataListOrder}
          onRowMoved={e => this._moveOrderItem(this, e.from, e.to)}
